...
WINDOW_SEC = 10.0       # segundos visibles en la gráfica
...
def main():
    ...
    mx   = deque(maxlen=maxlen); my   = deque(maxlen=maxlen); mz   = deque(maxlen=maxlen)

    # --- Layout: texto arriba + 3 plots abajo ---
    fig = plt.figure()
    ...
    # MAG
    m1, = ax3.plot([], [], label="mx_uT")
    m2, = ax3.plot([], [], label="my_uT")
    m3, = ax3.plot([], [], label="mz_uT")
    ax3.set_ylabel("mag [uT]")
    ...
    def update(_frame):
        ...
        if last_sample is None:
            text_artist.set_text("Esperando datos... (¿tu ESP32 envía líneas con \\n?)")
            return (text_artist, l1, l2, l3, g1, g2, g3, m1, m2, m3)
        ...
        mx.append(s["mx_uT"]);  my.append(s["my_uT"]);  mz.append(s["mz_uT"])
        ...
        m1.set_data(t, mx);   m2.set_data(t, my);   m3.set_data(t, mz)
