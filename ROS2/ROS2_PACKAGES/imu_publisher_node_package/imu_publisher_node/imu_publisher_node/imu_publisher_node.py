...
# --- Madgwick + numpy for orientation estimation (IMU + MAG) ---
try:
    import numpy as np
...
# Frecuencia aproximada de muestreo del ESP32 (en Hz)
SAMPLING_RATE = 100.0
...
def main(args=None):
    ...
    print("✅ ROS2 node 'simple_imu_publisher' starting with Madgwick MARG (IMU + MAG)...", flush=True)

    # -------- Madgwick (IMU + MAG) --------
    # Cuaternión inicial
    q = np.array([1.0, 0.0, 0.0, 0.0], dtype=float)
    # Filtro con frecuencia fija y beta bajo para más suavidad
    madgwick = Madgwick(beta=0.03, frequency=SAMPLING_RATE)

    def update_orientation(sample: dict):
        """Actualiza el cuaternión global q usando acelerómetro + giroscopio + magnetómetro."""
        nonlocal q, madgwick
        ...
        # Magnetómetro [uT] (dirección es lo importante; la escala no tiene que ser exacta)
        ...
        try:
            # Madgwick MARG (Mag + Acc + Gyro)
            new_q = madgwick.updateMARG(q, gyr=gyr, acc=acc, mag=mag)
            if new_q is not None:
                new_q = np.array(new_q, dtype=float)

                # --- suavizado extra del cuaternión para quitar jitter ---
                # 0.0 = nada, 1.0 = Madgwick puro; 0.1–0.3 suele ser razonable
                alpha = 0.2
                q_smoothed = (1.0 - alpha) * q + alpha * new_q

                # normalizar por si acaso
                norm = np.linalg.norm(q_smoothed)
                if norm > 0:
                    q[:] = q_smoothed / norm
        except Exception as e:
            print(f"[WARN] Madgwick MARG update error: {e}", flush=True)
    ...
    print("✅ Publishing Imu on /imu/data (orientation from Madgwick MARG)\n", flush=True)
